...
int main()
{
	...
	try
	{
		while (true)
		{
			...
			HANDLE hPipe = CreateNamedPipe(L"\\\\.\\pipe\\Tube", PIPE_ACCESS_DUPLEX, PIPE_TYPE_MESSAGE | PIPE_WAIT, 1,
				NULL, NULL, INFINITE, NULL);				
			...
			while (ReadFile(hPipe, rbuf, sizeof(rbuf), &cbRead, NULL) != FALSE)
			{
				rbuf[cbRead] = '\0';
				puts(rbuf);
				
				WriteFile(hPipe, wbuf, sizeof(wbuf), &cbWritten, NULL);
